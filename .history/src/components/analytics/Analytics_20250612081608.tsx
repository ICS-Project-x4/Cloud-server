    ...
    const fetchAnalyticsData = async () => {
      try {
        ...
        const filteredMessages = messages.filter(msg => 
          new Date(msg.created_at).getTime() > now.getTime() - timeRangeMs
        );

        // Calculate basic stats
        const totalMessages = filteredMessages.length;
        const deliveredMessages = filteredMessages.filter(msg => msg.status === 'delivered').length;
        const pendingMessages = filteredMessages.filter(msg => msg.status === 'pending').length;
        const failedMessages = filteredMessages.filter(msg => msg.status === 'failed').length;
        
        // Calculate revenue from transactions
        const revenue = filteredMessages.reduce((sum, msg) => 
        ...
        setAnalyticsData({
          ...
          revenue,
          activeUsers: new Set(filteredMessages.map(msg => msg.user_id)).size,
