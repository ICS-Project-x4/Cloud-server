...
export default function SMSCenter() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const todayMessages = response.filter(msg => 
        msg.created_at.startsWith(today)
      );
      
      const sentToday = todayMessages.filter(msg => msg.direction === 'outbound').length;
      const receivedToday = todayMessages.filter(msg => msg.direction === 'inbound').length;
      const pendingMessages = response.filter(msg => msg.status === 'pending').length;
      
      const totalSent = response.filter(msg => msg.direction === 'outbound').length;
      const deliveredCount = response.filter(msg => 
