  ...
  useEffect(() => {
    ...
    const fetchSMSStats = async () => {
      try {
        ...
        // Calculate message status statistics
        const deliveredCount = smsList.filter((msg: SMS) => msg.status === 'delivered').length;
        const pendingCount = smsList.filter((msg: SMS) => msg.status === 'pending').length;
        const failedCount = smsList.filter((msg: SMS) => msg.status === 'failed').length;
              ...
              <div className="mt-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-green-400 font-medium">
                    {messageStats.total > 0 ? ((messageStats.delivered / messageStats.total) * 100).toFixed(1) : 0}%
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                  ...
                  <span className="text-yellow-400 font-medium">
                    {messageStats.total > 0 ? ((messageStats.pending / messageStats.total) * 100).toFixed(1) : 0}%
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
